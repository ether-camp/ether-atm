...
function processData(data) {
  ...
  if (data.exchangeRate) setExchangeRate(data.exchangeRate, data.exchangeRateEth);
  if (data.buyerAddress) setBuyerAddress(data.buyerAddress);
  if (data.credit)
    setCredit(data.credit.fiat, data.credit.bitcoins || data.credit.ethers, data.credit.lastBill);
  ...
  switch (data.action) {
    ...
    case 'scanStart':
      if (coin == 'BTC') {
          ...
          setState('scan_address_eth');
      }
      break;
    case 'scanned':
      confirmBeep.play();
      setExchangeRateFromMemory();
      if (coin == 'BTC') {
          setState('insert_bills');
      } else {
          setState('insert_bills_eth');
      }
      break;
    ...
    case 'bitcoinTransferPending':
      ...
      break;
    case 'etherTransferPending':
      setState('sending_coins_eth');
      break;
    case 'bitcoinTransferComplete':
      confirmBeep.play();
      setState('completed');
      break;
    case 'etherTransferComplete':
      confirmBeep.play();
      setState('completed_eth');
      break;
...
$(document).ready(function () {
  ...
  touchEvent(startButtons, function(e) {
    ...
    coin = startButtonJ.attr('data-coin');
    var newLocale = startButtonJ.data('locale');
    if (coin == 'BTC') {
        setState('scan_address', null, newLocale);
    } else {
        setState('scan_address_eth', null, newLocale);
    }
    buttonPressed('start', coin);
  });
  ...
  var sendCoinsEthButton = document.getElementById('send-coins-eth');
  touchEvent(sendCoinsEthButton, function() {
    setState('sending_coins_eth');
    buttonPressed('sendEthers');
  });
  ...
  var insertBillCancelButton_eth = document.getElementById('insertBillCancel-eth');
  touchEvent(insertBillCancelButton_eth, function() {
    setBuyerAddress(null);
    buttonPressed('cancelInsertBill');
  });
  ...
  setupButton('scanCancel-eth', 'cancelScan');
  setupButton('completed_viewport', 'completed');
  setupButton('completed_viewport-eth', 'completed');
...
function setState(state, delay, newLocale) {
  ...
  if (state === 'insert_more_bills_eth') {
    $('#limit-reached-section-eth').css({'display': 'none'});
    $('#insert-another').css({'display': 'block'});
    t('or', locale.translate('OR').fetch());
    $('.or-circle circle').attr('r', $('#js-i18n-or-eth').width() / 2 + 15);
  }
...
function setLocale(data) {
  ...
  if (lastRate) setExchangeRate(lastRate, lastRateEth);
}
...
function setPrimaryLocales(primaryLocales) {
  ...
  if (primaryLocales.length === 1) {
    ...
    langCircles.html('<div class="circle-button"><span class="js-i18n-' +
      currentLocale + ' solo">BITCOIN</span></div>');
...
function setCredit(fiat, bitcoins, lastBill) {
  ...
  if (currentState === 'insert_bills_eth') setState('insert_more_bills_eth');
  ...
  if (coin == 'BTC') {
      ...
      updateEthers('.total-eth-rec', bitcoins);
  }
}
...
function updateEthers(selector, ethers) {
  var units = 'ETH';
  var adjustedValue = ethers;
  var formatted = formatEthers(adjustedValue);
  $(selector).find('.btc-amount').html(formatted);
  $(selector).find('.bitcoin-units').html(units);
}
...
function setExchangeRate(rate, rateEth) {
  lastRate = rate;
  lastRateEth = rateEth;
  var rateStr = formatFiat(rate.xbtToFiat, 2);
  var rateStr_eth = formatFiat(rateEth.ethToFiat, 2);
  ...
  var translated_eth = locale.translate('Our current Ether price is %s').fetch(rateStr_eth);
  $('.js-i18n-current-ether-price').html(translated_eth);
  if (coin == 'BTC') {
      updateBitcoins('.reverse-exchange-rate', rate.fiatToXbt);
  } else {
      updateEthers('.reverse-exchange-rate', rateEth.fiatToEth);
  }
...
function setExchangeRateFromMemory() {
    try {
      if (!lastRate || !lastRateEth) return;
      var rate = lastRate;
      var rateEth = lastRateEth;
      var rateStr = formatFiat(rate.xbtToFiat, 2);
      var rateStr_eth = formatFiat(rateEth.ethToFiat, 2);
      var translated = locale.translate('Our current Bitcoin price is %s').fetch(rateStr);
      $('.js-i18n-current-bitcoin-price').html(translated);
...
function sendOnly(reason) {
  ...
  if (coin == 'BTC') {
      ...
      var reasonText = reason === 'transactionLimit' ?
        'Transaction limit reached.' :
        'We\'re out of ethers.';
      t('limit-reached-eth', locale.translate(reasonText).fetch());
      t('limit-description-eth',
        locale.translate('Please touch <strong>Send Ethers</strong> to complete your purchase.').fetch());
      $('#insert-another-eth').css({'display': 'none'});
      $('#limit-reached-section-eth').css({'display': 'block'});
  }
}
